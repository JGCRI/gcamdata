...
module_gcam.china_L2231.electricity_nuclear_CHINA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L223.StubTechFixOut_elec_nuclear_CHINA <- fixed_nuclear %>%
      gather(year, fixedOutput, -province.name) %>%
      mutate(supplysector = "electricity", subsector = "nuclear", stub.technology = "Gen_III", subsector.share.weight = 0, share.weight = 0) %>%
      left_join(province_names_mappings %>% select(province, province.name), by = "province.name") %>%
      mutate(year = as.numeric(substr(year, 2, 5)), share.weight.year = year) %>%
      select(-province.name) %>%
      rename(region = province) %>%
      mutate(fixedOutput = if_else(fixedOutput < 0, 0, fixedOutput)) %>%
      select(region, supplysector, subsector, stub.technology, year, fixedOutput, share.weight.year, subsector.share.weight, share.weight) %>%

    # ===================================================
    # Produce outputs
    L2231.electricity_nuclear_CHINA() %>%
      ...
      add_legacy_name("L223.StubTechFixOut_elec_nuclear_CHINA") %>%
