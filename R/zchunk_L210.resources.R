...
module_energy_L210.resources <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    A10.TechChange_SSPs <- get_data(all_data, "energy/A10.TechChange_SSPs") %>%
      gather_years
    A10.EnvironCost_SSPs <- get_data(all_data, "energy/A10.EnvironCost_SSPs", ensure_currency_year = FALSE) %>% # year for data not provided
      gather_years
    ...
    L210.ReserveCalReserve.uncon_other_reg <- L210.ReserveCalReserve %>%
                                              ...
                                              anti_join(L210.ReserveCalReserve_unoil, by = "region") %>%
