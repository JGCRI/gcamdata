  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    # removed from the data frame.
     ...
     L142.pfc_R_S_T_Yh_future %>%
       ...
       mutate(input.emissions = round(input.emissions, emissions.DIGITS), year = as.numeric(year))->
       L241.pfc_all
     ...
     L141.hfc_ef_R_cooling_Yh %>%
       filter(year == max(emissions.HFC_BASE_YEARS)) %>%
       left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") ->
     ...
     L241.hfc_cool_ef_update_all %>%
       gather(year, value, grep(YEAR_PATTERN, names(.))) %>%
       filter(!year %in% emissions.HFC_BASE_YEARS) ->
     ...
     L241.hfc_ef_2010_update_all %>%
       ...
       mutate(emiss.coeff = round(value, emissions.DIGITS), year = as.numeric(year)) %>%
