...
module_gcam.china_L254.transportation_china <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L154.in_EJ_province_trn_m_sz_tech_F <- get_data(all_data, "L154.in_EJ_province_trn_m_sz_tech_F")
    L154.out_mpkm_province_trn_nonmotor_Yh <- get_data(all_data, "L154.out_mpkm_province_trn_nonmotor_Yh")
    ...
    L254.StubTranTechCalInput_CHINA %>%
      ...
      mutate(output = round(calibrated.value * loadFactor * CONV_EJ_GJ / (coefficient * CONV_BTU_KJ),
                            digits = gcamchina.DIGITS_TRNCHINA_DEFAULT))  ->
