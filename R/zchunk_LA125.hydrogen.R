...
module_energy_LA125.hydrogen <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    H2A_prod_cost <- get_data(all_data, "energy/H2A_NE_cost_data", ensure_currency_year = FALSE) %>% # multiple currency years, so need to convert manually
      select(-notes) %>%
      gather_years() %>%
      mutate(value = if_else(units == "$2016/kg H2", value * gdp_deflator(CURRENCY_YEAR, 2016),
                             if_else(units == "$2005/kg H2", value * gdp_deflator(CURRENCY_YEAR, 2005),
                                     NA_real_)),
             units = paste0("$", CURRENCY_YEAR, "/kg H2"))
    ...
    # C. Costs: Calculate max improvement of nuclear power generation capital overnight costs
     ...
     # D. Convert Units from H2A ($/kg, GJ/kg) to GCAM (CURRENCY_YEAR$/GJ, GJ/GJ)
     H2A_prod_cost %>%
       mutate(value = value / CONV_GJ_KGH2,
              units = paste0("$", CURRENCY_YEAR, "/GJ H2")) -> H2A_prod_cost_conv
    ...
    L125.Electrolyzer_IdleRatio_Params %>%
      ...
      add_units(paste0(CURRENCY_YEAR, "$/kg H2")) %>%
