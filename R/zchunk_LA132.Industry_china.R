...
module_gcam.china_LA132.Industry <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    fuel <- year <- province <- value <- sector <- GCAM_region_ID <- refinery_comsumption <- EBProcess <-
    ...
    L101.inNBS_Mtce_province_S_F %>%
      ...
      complete(province, year = HISTORICAL_YEARS) %>%
    ...
    L132.in_pct_province_ind_F %>%
      filter(fuel %in% L123.in_EJ_R_indchp_F_Yh$fuel) %>%
