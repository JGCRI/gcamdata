#' module_gcam.china_LB126.Gas_ElecTD_china
#'
#' Calculates inputs and outputs of: gas processing by fuel and province, gas pipeline by province, and transmission and distribution of electricity by province.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{L126.out_EJ_province_pipeline_gas}, \code{L126.in_EJ_province_pipeline_gas}, \code{L126.out_EJ_province_gasproc_F}, \code{L126.in_EJ_province_gasproc_F}, \code{L126.out_EJ_province_td_elec}, \code{L126.in_EJ_province_td_elec}. The corresponding file in the
#' original data system was \code{LB126.Gas_ElecTD.R} (gcam-china level1).
#' @details Calculates inputs and outputs of: gas processing by fuel and province, gas pipeline by province, and transmission and distribution of electricity by province.
#' @importFrom assertthat assert_that
#' @importFrom dplyr filter mutate select
#' @importFrom tidyr gather spread
#' @author YangLiu September 2018
module_gcam.china_LB126.Gas_ElecTD_china <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L126.out_EJ_province_pipeline_gas",
             "L126.in_EJ_province_pipeline_gas",
             "L126.out_EJ_province_gasproc_F",
             "L126.in_EJ_province_gasproc_F",
             "L126.out_EJ_province_td_elec",
    ...
    # Deriving gas pipeline energy use (net) from national estimate and each province's share
    L126.net_EJ_china_pipeline <- L126.in_EJ_R_gaspipe_F_Yh %>%
    ...
    # Calculate net pipeline energy use as total value * province shares of pipeline energy use
    L126.net_EJ_province_pipeline_gas <- L101.inNBS_Mtce_province_S_F %>%
      ...
      ungroup() %>%
      # Apportion to provinces
      left_join_error_no_match(L126.net_EJ_china_pipeline, by = c("sector", "fuel", "year")) %>%
      # province value = province share * china total
      transmute(value = value.x * value.y, province, sector, fuel, year)

    # PIPELINE OUTPUT AND INPUT

    # Pipeline "output" is equal to each province's total gas consumption by all sectors
    ...
    # Pipeline output
    L126.out_EJ_province_pipeline_gas <- L126.in_EJ_province_F %>%
      filter(fuel == "gas") %>%
      mutate(sector = "gas pipeline") %>%
      select(province, sector, fuel, year, value)

    # Gas pipeline input = output plus pipeline energy use
    L126.in_EJ_province_pipeline_gas <- L126.out_EJ_province_pipeline_gas %>%
      left_join_error_no_match(L126.net_EJ_province_pipeline_gas,
    ...
    # NOTE: Coal gasification (town gas) is disaggregated to provinces on the basis of industrial coal consumption
    L126.pct_province_gasproc_coal <- L101.inNBS_Mtce_province_S_F %>%
      filter(sector == "industry",
             fuel == "coal") %>%
      mutate(sector = "gas processing") %>%
      group_by(year) %>%
      # province share = province value / total value
      mutate(value = value / sum(value))

    # Apportion nation-level data to provinces
    L126.in_EJ_province_gasproc_coal <- L126.pct_province_gasproc_coal %>%
      left_join_error_no_match(L122.in_EJ_R_gasproc_F_Yh, by = c("sector", "fuel", "year")) %>%
      # province input energy = province share * china input energy
      mutate(value = value.x * value.y) %>%
      select(province, sector, fuel, year, value)

    L126.out_EJ_province_gasproc_coal <- L126.pct_province_gasproc_coal %>%
      left_join_error_no_match(L122.out_EJ_R_gasproc_F_Yh, by = c("sector", "fuel", "year")) %>%
      # province output energy = province share * china output energy
      mutate(value = value.x * value.y) %>%
      select(province, sector, fuel, year, value)

    # Biomass gasification inputs and outputs
    # NOTE: Biomass gasification (biogas) is disaggregated to provinces on the basis of electric sector waste biomass consumption
    L126.in_pct_province_gasproc_bio <- L101.EIA_use_all_Bbtu %>%
      ...
      mutate(fuel = "biomass") %>%
      group_by(year) %>%
      # province share in each year = province value / total value
      mutate(value = value / sum(value)) %>%
      ungroup()

    # Multiply national totals by province shares to get each province's scaled biomass gasification outputs and inputs
    L126.out_EJ_province_gasproc_bio <- L126.in_pct_province_gasproc_bio %>%
      left_join_error_no_match(L122.out_EJ_R_gasproc_F_Yh, by = c("fuel", "year")) %>%
      # province output energy = province share * china output energy
      mutate(value = value.x * value.y) %>%
      select(province, sector, fuel, year, value)

    L126.in_EJ_province_gasproc_bio <- L126.in_pct_province_gasproc_bio %>%
    ...
    # The remainder of each province's consumption of gas is assigned to the natural gas technology
    L126.out_EJ_province_gasproc_gas <- L126.in_EJ_province_pipeline_gas %>%
      left_join_error_no_match(L126.out_EJ_province_gasproc_coal, by = c("province", "year")) %>%
      left_join_error_no_match(L126.out_EJ_province_gasproc_bio, by = c("province", "year")) %>%
      # Keep province and year columns, set sector, fuel and value columns
      transmute(province,
    ...
    L126.in_EJ_province_gasproc_gas <- L126.out_EJ_province_gasproc_gas

    # Bind the input and output tables of gas processing technologies
    L126.out_EJ_province_gasproc_F <- bind_rows(L126.out_EJ_province_gasproc_gas, L126.out_EJ_province_gasproc_bio, L126.out_EJ_province_gasproc_coal)
    L126.in_EJ_province_gasproc_F <- bind_rows(L126.in_EJ_province_gasproc_gas, L126.in_EJ_province_gasproc_bio, L126.in_EJ_province_gasproc_coal)
    ...
    # Produce outputs
    L126.out_EJ_province_pipeline_gas %>%
      add_title("Output of gas pipeline sector by province") %>%
      add_units("EJ") %>%
      add_comments("Sum of final energy input from gas in china provinces") %>%
      add_legacy_name("L126.out_EJ_province_pipeline_gas") %>%
    ...
    L126.in_EJ_province_pipeline_gas %>%
      add_title("Input to gas pipeline sector by province") %>%
      add_units("EJ") %>%
      add_comments("Values from L126.out_EJ_province_pipeline_gas plus net energy values") %>%
      add_legacy_name("L126.in_EJ_province_pipeline_gas") %>%
      same_precursors_as(L126.out_EJ_province_pipeline_gas) ->
      L126.in_EJ_province_pipeline_gas

    L126.out_EJ_province_gasproc_F %>%
      add_title("Output of gas processing sector by province and technology") %>%
      add_units("EJ") %>%
      add_comments("Coal and biomass gas calculated by apportioning national values to provinces") %>%
      add_comments("Natural gas calculated by subtracting coal and biomass gas from total pipeline input") %>%
      add_legacy_name("L126.out_EJ_province_gasproc_F") %>%
    ...
    L126.in_EJ_province_gasproc_F %>%
      add_title("Inputs to gas processing sector by province and technology") %>%
      add_units("EJ") %>%
      add_comments("Coal and biomass gas calculated by apportioning national values to provinces") %>%
      add_comments("Natural gas calculated by subtracting coal and biomass gas from total pipeline input") %>%
      add_legacy_name("L126.in_EJ_province_gasproc_F") %>%
    ...
    L126.out_EJ_province_td_elec %>%
      add_title("Output of electricity T&D sector by province") %>%
      add_units("EJ") %>%
      add_comments("Sum of all tracked demands of electricity") %>%
      add_legacy_name("L126.out_EJ_province_td_elec") %>%
    ...
    L126.in_EJ_province_td_elec %>%
      ...
      add_legacy_name("L126.in_EJ_province_td_elec") %>%
      add_precursors("L122.in_EJ_province_refining_F",
                     "L123.out_EJ_province_elec_F",
                     "L132.in_EJ_province_indchp_F",
                     "L132.in_EJ_province_indfeed_F",
    ...
    return_data(L126.out_EJ_province_pipeline_gas, L126.in_EJ_province_pipeline_gas, L126.out_EJ_province_gasproc_F, L126.in_EJ_province_gasproc_F, L126.out_EJ_province_td_elec, L126.in_EJ_province_td_elec)
  } else {
