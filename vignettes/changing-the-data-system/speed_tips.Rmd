...
* Avoid `filter(x, z %in% y$z)`, instead using `semi_join(y, by = "z")` or `anti_join(y, by = "z")`. This is usually 25-30% faster. The exception is when making multiple filter calls, in which case it is often faster to have one call to `filter()` rather than use `filter() %>% semi_join()`.
