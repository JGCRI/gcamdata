...
output_to_save <- load_from_cache("L2072.AgCoef_BphysWater_bio_mgmt")
save_chunkdata(output_to_save)
```
...
chunk_outputs <- driver_drake(return_outputs_of = "module_energy_L244.building_det")
save_chunkdata(chunk_outputs)
```
...
# Load all outputs
all_outputs <- load_from_cache(all_output_names$target)
save_chunkdata(all_outputs)
```
