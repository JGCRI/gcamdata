...
User-modification chunks have a format similar to other data chunks in gcamdata, except that instead of producing a new output, it returns a modified data object that replaces the original data object in the data system. These new chunks can be added to `driver_drake()` or `driver()` using the new arguments `user_modifications` and `user_suffix`, which tell gcamdata which modification function to use and what suffix to add to all impacted downstream objects and xmls. 
...
usermod_fert <- function(command, ...) {
  if(command == driver.DECLARE_MODIFY) {
    ...
    # NOTE: when modifying FILE inputs the object name includes the path to the file,
    # i.e. "energy/A23.globaltech_eff" not "A23.globaltech_eff" thus we will have to
    # explicitly "name" it to make sure it is set correctly.  Other objects can be
    # listed out just like for `return_data`
...
for (i in 1:length(shareweights)){
  ...
  driver(user_modifications = c("usermod_fert"),
               user_suffix = paste0("__", i))
}
```
